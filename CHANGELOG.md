...
- **Host port exposure** — Postgres (`5432`) and the debug/monitoring ports (Flower `5555`, gunicorn `8000`, supervisord `9001`) now bind to localhost by default (`POSTGRES_BIND` / `POSTGRES_HOST_PORT` / `DEBUG_BIND` env vars), fixing collisions with a host-installed Postgres and preventing unauthenticated public exposure on VPS deployments.

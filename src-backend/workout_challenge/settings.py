...
# the cleartext-on-public-bind startup warning below - the actual bind is
...
def _assert_distinct_operational_secrets():
    """Refuse production boot when Flower/OW/Health passwords equal SECRET_KEY.

    Reusing Django's SECRET_KEY as a dashboard/developer password couples
    session-forging material to operational UIs. DEBUG skips the check so
    local setups stay frictionless.
    """
    if DEBUG:
        return
    offenders = []

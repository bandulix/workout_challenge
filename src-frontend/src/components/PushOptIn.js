...
function PushOptInCard({compact = false}) {
    ...
    if (platform.isIOS && !platform.standalone) {
        return (
            <div className="rounded-3xl border border-ink-700/60 bg-ink-850 text-white p-5">
                <div className="flex items-center gap-3 mb-3">
                    <div className="h-10 w-10 rounded-2xl bg-volt-400/15 flex items-center justify-center">
                        <Smartphone className="h-5 w-5 text-volt-400"/>
                    </div>
                ...
                <ol className="mt-3 space-y-2 text-sm text-gray-200">
                    <li className="flex items-center gap-2">
                        <span className="text-volt-400 font-bold">1.</span> Tap <Share className="inline h-4 w-4 -mt-0.5"/> <b>Share</b> in Safari
                    </li>
                    <li className="flex items-center gap-2">
                        <span className="text-volt-400 font-bold">2.</span> <PlusSquare className="inline h-4 w-4 -mt-0.5"/> <b>Add to Home Screen</b>
                    </li>
                    <li className="flex items-center gap-2">
                        <span className="text-volt-400 font-bold">3.</span> Open the app from your home screen
                    </li>
                </ol>
            </div>
        );
    ...
    return (
        <div className="rounded-3xl border border-ink-700/60 bg-ink-850 text-white p-5">
            <div className="flex items-center gap-3 mb-2">
                <div className="h-10 w-10 rounded-2xl bg-volt-400/15 flex items-center justify-center">
                    {subscribed ? <BellRing className="h-5 w-5 text-volt-400"/> : <Bell className="h-5 w-5 text-volt-400"/>}
                </div>
                <div>
            ...
            {!platform.standalone && (platform.isAndroid || window.deferredInstallPrompt) && (
                <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 rounded-2xl border border-volt-400/40 text-volt-300 py-2.5 text-sm font-semibold hover:bg-volt-400/10 transition"
                >
